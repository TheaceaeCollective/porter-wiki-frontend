...
<template>
    <RouterLink :to="post.url || '/not-found'" class="w-fit">
        ...
        <OverlapGrid class="big-news-post w-full h-full rounded-lg overlap-grid md:w-[598px] md:h-[502px]" v-if="post.meta">
            ...
            <div class="card-content">
                <div class="gap-4">
                    ...
                    <LoadingImage
                        :src="post.meta.image || DefaultImage || EmptyImage"
                        class="object-cover w-64 h-64 self-center rounded-lg"
                    />
                    <h2 class="text-3xl italic font-semibold self-center text-center text-shadow">{{ post.meta.title }}</h2>
                </div>
                <div>
                    <p class="post-description w-full leading-tight self-center text-center">{{ post.meta.description }}</p>
                </div>
            </div>
        </OverlapGrid>
    </RouterLink>
...
.big-news-post {
    ...
    &:hover > .dim {
        ...
        background: rgba(0, 0, 0, 0) linear-gradient(180deg, transparent 0%, var(--background-1) 85%);
    }
