    ...
    <RouterLink v-if="post.url" :to="post.url || '/not-found'">
        ...
        >
            <div class="flex justify-between items-center">
                <h1 class="simple-post-title text-3xl font-medium">
                    {{ post.meta.title || "post title" }}
                </h1>
                <h3 class="text-base font-medium text-nowrap">
                    {{ Formatting.formatDate(post.meta.date) }}
                </h3>
            </div>
            <div class="w-full flex flex-col gap-3">
                <p
                    class="text-lg w-full leading-tight break-words whitespace-pre-wrap"
                >
