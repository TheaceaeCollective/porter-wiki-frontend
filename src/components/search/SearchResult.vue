...
<template>
    <RouterLink @click="resultClick" :to="result.url || '/not-found'">
        <OverlapGrid class="search-result w-full h-28 rounded-lg overlap-grid" v-if="result.meta">
            <LoadingImage v-if="result.meta.image" :src="result.meta.image || EmptyImage" class="object-cover" />
            ...
            <div class="flex flex-col h-auto justify-between gap-2 p-5">
                <div>
                    <div class="flex justify-between items-center">
                        <p class="flex gap-0.5">
                            ...
                            <span v-for="(part, index) in result.breadcrumbs" class="flex items-center gap-1">
                                ...
                                <RouterLink
                                    v-if="index != Object.keys(result.breadcrumbs).length - 1"
                    ...
                    <h2 class="post-title text-xl font-medium leading-6 text-shadow truncate max-md:max-w-48">{{ result.meta.title }}</h2>
                </div>
                <div>
                    <div>
...
.search-result {
    ...
    &:hover > .dim {
        ...
        background: rgba(0, 0, 0, 0) linear-gradient(-180deg, transparent 0%, var(--background-2) 75%);
    }
